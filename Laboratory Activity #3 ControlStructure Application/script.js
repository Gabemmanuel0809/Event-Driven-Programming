...
let m = document.querySelector("#validationMessage");
let summ = document.querySelector("#orderSummary");

const calculateAmount = (price, quantity)=> {return price * quantity};

const calculateDiscount = (subtotal) => {
    let discount;
    if(subtotal >= 5000) {
        discount = 0.10;
    } else if(subtotal >= 3000 && subtotal <= 4999) {
        discount = 0.7;
...
const getDeliveryTypeName = (option) => {
  ...
  calculate.addEventListener("click", function () {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const itemAmount = calculateAmount(pPrice, pQty);
      subtotal += itemAmount;
  
      productListHTML += `
        <p><strong>${i + 1}. ${pName}</strong><br>
        Price: ₱${pPrice.toFixed(2)}<br>
        Quantity: ${pQty}<br>
        Amount: ₱${itemAmount.toFixed(2)}</p>
      `;
    }
